...
if __name__ == "__main__":
    ...
    # 哨兵带 usage：既能 is 验身份，又能验 token 提取
    ...
    try:
        ...
        assert (m.prompt_tokens, m.completion_tokens) == (10, 5), "usage 提取错了"
        assert m.latency_s >= 0

        # ② FATAL 一击即抛 → 总调用 1，异常原样透传（不翻译成 LLMUnavailable）
        ...
        # ③ 全程失败 → 总调用 4，抛 LLMUnavailable，账本随身，异常链保留元凶
        ...
        try:
            ...
            assert isinstance(e.__cause__, openai.APIConnectionError), "from exc 丢了元凶"
            assert (e.metrics.attempts, e.metrics.outcome) == (4, "gave_up")
        ...
        resp, m = chat_with_retry(fake, model="m", messages=[])
        assert resp is SENTINEL and fake.calls == 3
        ...
        resp, m = chat_with_retry(fake, model="m", messages=[])
        assert resp is SENTINEL and len(sleeps) == 2
        assert 0 <= sleeps[0] <= 1.0 and 0 <= sleeps[1] <= 2.0, f"应退回退避区间，实际 {sleeps}"

        # ⑥ 预算不够睡 → 不睡，立刻抛 LLMUnavailable，总调用 1
        sleeps.clear()
        ...
        try:
            ...
            assert isinstance(e.__cause__, openai.RateLimitError), "from exc 丢了元凶"
            assert (e.metrics.attempts, e.metrics.outcome) == (1, "gave_up")
        assert fake.calls == 1, f"期望 1 次调用，实际 {fake.calls}"
        assert sleeps == [], f"睡不起就别开始睡，实际睡了 {sleeps}"
    finally:
        time.sleep = _real_sleep  # 冒烟归冒烟，全局的东西借了要还

    print("chat_with_retry: 6/6 冒烟通过（含块 5 metrics/异常翻译断言）")
