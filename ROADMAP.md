...
> 最近更新：2026-08-10（用户拍板：**先 L13，随后「模型后训练」专项 SFT+RL 一起提前**；此前 L11/L12/Langfuse 已封版）
...
| 模型后训练 **SFT+RL(PPO/DPO) ⬆️整体提前（2026-08-10 用户拍板：L13 之后作专项课，动手=LoRA-SFT+DPO）**；工具内化仍留此池 | 🟩 | **Ch7**（16 项目） | JD3 加分 |
...
5. ✅ **L12 评估**（书 Ch6，2026-08-04 封版）——Rubric LLM-judge 填 L7 子串匹配坑 + 金标集校准 + 统计显著性。🟨
6. ✅ **可观测性 Langfuse**（capstone LangGraph 版接入,L13 预习）。🟦
7. 🚧 **L13 可观测/成本/延迟 + API 重试限流 + L13.5 Harness**（**下一个主推**）。🟦
8. ⬜ **模型后训练专项 SFT+RL**（书 Ch7，**2026-08-10 用户拍板提前**；小模型 LoRA-SFT + DPO，复用 eval/rubric_judge 做 before/after 闭环）。🟩
9. ⬜ **L14 部署深化**（K8s）。🟦
