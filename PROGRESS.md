...
- [ ] **L2 手写 agent loop** —— 讲义/quiz/作业骨架已给；quiz 未答、代码作业未做  ← **当前断点**
- [ ] L3 tool use 深入
...
- **立刻要做**：让用户完成 L2 —— 先口头答 quiz 3 题（四步机制 / arguments 类型 / 为何要轮数上限），
  再写 `lesson-02/homework/order_agent.py`（查订单迷你客服 agent）。给反馈、判达标后进 L3。
- **需要注意**：坚持 coaching 式，给提示不给答案；每节课**封版后**才生成 PDF 且要反映真实对话；
  每次 commit 后 `git push` 到 B6nux9。
