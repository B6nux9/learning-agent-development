...
### 🔵 阶段二：核心能力补齐（投递前必做）
- [ ] **L6 上下文管理 + Prompt/Context Engineering** —— **未开始**，等用户开口才教（勿抢跑）　← **当前断点**
      （JD 4/10 + 5/10，合并成一节；沃孚 JD 就是把 Prompt/Context/Harness Engineering 并列的）
      **⚠️ L6 必须额外包含「结构化输出 / 输出控制」**（补 L1-L5 缺口 2；蔚蓝 JD 写"输出控制"、
      拼多多写"输出过滤"）：JSON mode / structured output / schema 强制。**回扣他在 L4 LLM router 里
      的土办法**（system prompt 约束 + `VALID_GROUPS` 校验兜底）——他已有直觉，补起来快。
      另：L6 开头花几分钟带他过一遍 `reference/模型选型.md`（补缺口 1+3：推理模型 vs 普通模型、
      主流厂商对比）。
...
- **立刻要做**：**什么都不要做，等用户说"开始 L6 / 继续"**。L5 已封版、JD 大纲重排已完成，主动权在用户手里。
- 用户开口后教 **L6 上下文管理 + Prompt/Context Engineering**（合并成一节）。这是他**从 L2 就最惦记**的
  问题，且 L5 已埋两处伏笔：①messages 越滚越长 ②长期记忆存不下。开课时明确点回
  "这就是你 L2 自己推导出、L4/L5 又亲手撞上的那个问题"。Prompt Engineering 部分可回扣 L5 的实战教训
  （"prompt 太软 → 模型不调工具"，改强命令后解决）。
